...
describe("AX v1alpha1 contract", () => {
  test.each(["Task", "Workspace", "Gateway", "Model"])(
    "accepts a minimal %s",
    (kind) => {
      expect(
        validate(
  ...
  test("requires a resource name", () => {
    expect(
      validate("apiVersion: ax.io/v1alpha1\nkind: Task\nmetadata: {}\n")[0]
        ?.ruleId,
    ).toBe("contract/required-field");
  });

  test("matches upstream defaults by allowing an omitted apiVersion", () => {
    expect(
      validate("kind: Task\nmetadata:\n  name: example\nspec: {}\n"),
    ).toEqual([]);
  });

  test("accepts upstream legacy Workspace and Model shapes", () => {
    expect(
      validate(
        "kind: Workspace\nmetadata:\n  name: repo\nspec:\n  mcp:\n    - name: tools\n      endpoint: http://tools\n",
      ),
