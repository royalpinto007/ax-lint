...
describe("parseAxSource", () => {
  test("parses multiple documents and preserves source locations", () => {
    ...
    expect(parsed.documents[1]?.location("metadata.name")).toMatchObject({
      file: "ax.yaml",
      line: 8,
      column: 9,
    });
  ...
  test("returns a located syntax diagnostic", () => {
    ...
    expect(parsed.diagnostics[0]).toMatchObject({
      ...
      severity: "error",
      location: { file: "broken.yaml", line: 2 },
    });
  });

  test("rejects duplicate mapping keys", () => {
    const parsed = parseAxSource(
      "kind: Task\nkind: Gateway\n",
      "duplicate.yaml",
    );
    expect(
      parsed.diagnostics.some((item) => item.ruleId === "contract/yaml-syntax"),
    ).toBe(true);
  });
